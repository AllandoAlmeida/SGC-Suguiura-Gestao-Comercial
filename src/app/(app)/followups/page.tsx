...
export default function FollowupsPage() {
  ...
  async function markContacted(lead: Lead) {
    try {
      await apiFetch(`/api/leads/${lead.id}/interactions`, {
        ...
        body: JSON.stringify({
          type: "LIGACAO",
          content: "Contato de follow-up realizado.",
        }),
      });
      load();
    } catch (e) {
  ...
  return (
    <div className="h-screen flex flex-col p-6 lg:p-8 overflow-hidden">
      <PageHeader
        title="Follow-ups"
        subtitle="Disciplina de acompanhamento — nenhum lead pode ficar sem contato"
      />
      ...
      <div className="flex flex-col overflow-y-auto">
        <Section
          ...
          emptyMsg="Nenhum follow-up atrasado. Otimo trabalho!"
        />
        <Section
          title="📅 Contatos de hoje"
          leads={today}
          onContacted={markContacted}
          emptyMsg="Nenhum contato agendado para hoje."
        />
        <Section
          title="🗓️ Proximos follow-ups"
          leads={upcoming}
          onContacted={markContacted}
  ...
  return (
    <div className="mb-8">
      <h2 className="font-semibold text-slate-800 mb-3">
        {title}{" "}
        <span className="text-slate-400 font-normal">({leads.length})</span>
      </h2>
      {leads.length === 0 ? (
        ...
        <div className="space-y-2">
          {leads.map((lead) => (
            <div
              key={lead.id}
              className="bg-white rounded-lg border border-slate-200 p-4 flex items-center gap-4 shadow-sm"
            >
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <Link
                    href={`/leads/${lead.id}`}
                    className="font-medium text-slate-800 hover:text-brand-600 hover:underline truncate"
                  >
                    {lead.name}
                  </Link>
                ...
                <p className="text-xs text-slate-500 mt-0.5">
                  {lead.phone} · {lead.product} ·{" "}
                  {formatCurrency(lead.estimatedValue)}
                </p>
              </div>
              <div className="text-right">
                <p
                  className={`text-sm font-medium ${isOverdue(lead.nextFollowUp) ? "text-red-600" : "text-slate-700"}`}
                >
              ...
              <button
                ...
                className="px-3 py-1.5 rounded-lg bg-green-600 text-white text-sm hover:bg-green-700 whitespace-nowrap"
              >
                ✓ Contatei
              </button>
            </div>
...
export default function FollowupsPage() {
  ...
  return (
    <div className="h-screen flex flex-col p-6 lg:p-8 overflow-hidden">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-3 mb-6 shrink-0">
        <SummaryCard label="Atrasados" value={overdue.length} tone="red" />
        <SummaryCard label="Para hoje" value={today.length} tone="amber" />
        <SummaryCard
          label="Proximos (em aberto)"
      ...
      {/* Corrigido: era h-screen (100vh) aninhado dentro de outro h-screen, o que
          empurrava o fim da lista pra fora da area visivel. Agora ocupa so o
          espaco que sobra (flex-1) e min-h-0 garante que o overflow-y-auto
          funcione de verdade dentro de um container flex. */}
      <div className="flex-1 min-h-0 overflow-y-auto">
        <Section
          title="⚠️ Follow-ups atrasados"
          leads={overdue}
          onContacted={setActionLead}
          emptyMsg="Nenhum follow-up atrasado. Otimo trabalho!"
        />
        <Section
          title="📅 Contatos de hoje"
