...
export default function LeadForm({ lead, users, onClose, onSaved }: Props) {
  ...
  const [form, setForm] = useState({
    ...
    nextFollowUp: toDateInput(lead?.nextFollowUp) || toDateInput(new Date()),
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!form.nextFollowUp) {
      setError("O proximo follow-up e obrigatorio.");
      return;
    }
    if (!form.ownerId) {
    ...
    try {
      if (isEdit) {
        await apiFetch(`/api/leads/${lead!.id}`, { method: "PATCH", body: JSON.stringify(payload) });
      } else {
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="p-6 grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <Field label="Telefone *">
            <input required value={form.phone} onChange={(e) => update("phone", e.target.value)} className={inputCls} placeholder="(11) 99999-9999" />
          </Field>

          <Field label="Origem *">
            <select value={form.source} onChange={(e) => update("source", e.target.value as LeadSource)} className={inputCls}>
              {SOURCES.map((s) => (
          ...
          <Field label="Produto *">
            <input required value={form.product} onChange={(e) => update("product", e.target.value)} className={inputCls} />
          </Field>

          <Field label="Valor estimado (R$)">
            <input type="number" min="0" step="0.01" value={form.estimatedValue} onChange={(e) => update("estimatedValue", e.target.value)} className={inputCls} />
          </Field>
          ...
          <Field label="Status">
            <select value={form.status} onChange={(e) => update("status", e.target.value as LeadStatus)} className={inputCls}>
              {STATUS_ORDER.map((s) => (
                <option key={s} value={s} disabled={statusOptionDisabled(s)}>
                  {STATUS_LABEL[s]}{statusOptionDisabled(s) ? " (bloqueado)" : ""}
                </option>
              ))}
            </select>
          </Field>
          <Field label="Proximo follow-up * (obrigatorio)">
            <input type="date" required value={form.nextFollowUp} onChange={(e) => update("nextFollowUp", e.target.value)} className={inputCls} />
          </Field>

          <Field label="Ultimo contato">
            <input type="date" value={form.lastContact} onChange={(e) => update("lastContact", e.target.value)} className={inputCls} />
          </Field>
          ...
          <div className="md:col-span-2 flex justify-end gap-2 pt-2">
            ...
            <button type="submit" disabled={saving} className="px-4 py-2 rounded-lg bg-brand-600 text-white hover:bg-brand-700 disabled:opacity-60">
...
const inputCls = "w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500";
